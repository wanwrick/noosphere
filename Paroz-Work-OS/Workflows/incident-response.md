...
4. **Assign Incident Commander (IC)** — for SEV-1/2, this is Paroz unless delegated
...
- [ ] Check DLT pipeline event log for error details
- [ ] Review Unity Catalog audit logs for recent changes
- [ ] Check cluster/warehouse compute health
- [ ] Verify source system status (Temenos, Kafka, APIs)
...
```
Data Engineer → Paroz (IC) → VP Engineering → CTO
                    ↓
